...
def sortwithloops(L):
    ...
    pos = L.index(min(L))
   # print (" is first min index ", pos)
    NewList=list()
    NewList.append(L.pop(pos)) #start with min value
    ...
    while len(L) != 0:
        for x in L:
            x = min(L)
            pos =L.index(min(L))
            NewList.append(L.pop(pos))#removes the item at position pos from L appends it to new list
            #print (NewList)
            #print L
